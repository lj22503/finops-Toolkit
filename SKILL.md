...
**路由决策（manifest.yaml slug 对照）：**

```
用户输入
    │
    ├─ "定方向"/"资源分配"/"季度规划" → 运营负责人 → manifest: operations-director
    │
    ├─ "增长策略"/"竞品分析"/"用户分层" → 策略运营 → manifest: strategy-ops
    │
    ├─ "写文案"/"做内容"/"投教" → 内容运营 → manifest: content-ops
    │
    ├─ "设计活动"/"活动策划" → 活动运营 → manifest: campaign-ops
    │
    ├─ "产品功能"/"运营工具"/"PRD" → 产品运营 → manifest: product-ops
    │
    ├─ "分析数据"/"复盘效果"/"监控" → 数据运营 → manifest: data-ops
    │
    ├─ "检查合规"/"合规审查"/"能发吗" → 合规运营 → manifest: compliance-ops
    │
    └─ 其他 → 匹配最相关角色，或问用户
```
